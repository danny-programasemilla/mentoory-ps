...
(function () {
    ...
    function openEditTemplate(btn) {
        ...
        var body = textField('tpl-name', 'Nombre', name, true, 200) +
                   textareaField('tpl-description', 'Descripción', description, 2000);
        openModal('Editar plantilla', body, function () {
            var payload = {
                externalId: ctx.externalId,
                name: val('tpl-name'),
                description: val('tpl-description') || null
            };
            if (!payload.name) { toast('El nombre es obligatorio.', 'warning'); return; }
            fetchJson(ctx.urls.updateTemplate, 'POST', payload)
                .then(function (d) { closeModal(); handleResponse(d, 'Plantilla actualizada.'); })
                .catch(networkError);
        });
    }
