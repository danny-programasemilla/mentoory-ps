...
| 15 | SC-005 | Audit trail — 3 coordinators | C5 | `WalkthroughAuditConcurrencyTests.AuditTrail_ThreeAdvancesByThreeCoordinators_AllNamesVisibleOnLifecyclePage` | ✅ (see note D) |
...
- **Note D (row 15 — SC-005 audit trail).** `Project.AdvanceStage` (`Mentoory.Tenant.Domain/Aggregates/Project/Project.cs:93`) stamps `AdvancedByUserId` on the stage being *started*, not the stage being *completed*. So after coord1, coord2, coord3 each advance in sequence, the Lifecycle page shows "Avanzada por: Ana Rodríguez" on Forms, "Luis Paredes" on Analysis, and "Sofía Navarro" on LearningAssignment (the currently-in-progress stage). Registration has no attribution because it was initialized by `Project.Create` without an acting user. The SC-005 "audit trail visible without navigation" guarantee is fully covered — three distinct coordinators' names appear on three different stages of the same project — even though the attribution row shifts by one compared to a naive "who completed this stage?" reading. Chunk C5 added seed user coord3 and a single role assignment in `Mentoory.Db.PostDeployment/004.SeedTestData.sql` to make this test possible.
