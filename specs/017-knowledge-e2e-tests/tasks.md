...
- [ ] T004 Extend `Mentoory.Db.PostDeployment/005.SeedKnowledgeData.sql` with one new `diagnostic.FormTemplates` row named `"Diagnóstico Básico de Emprendimiento"` (ExternalId `33333333-3333-3333-3333-333333333333`), bound to the seeded `Emprendimiento Básico` KS template, plus 2–3 `diagnostic.Questions` rows whose `TopicId`s resolve per T002's answer. Wrap every insert in `IF NOT EXISTS` guards following the existing idempotent pattern. Verify by running `dotnet build Mentoory.Db/MentooryDb.sqlproj`; it must publish without errors.
- [ ] T005 Extend `Mentoory.Db.PostDeployment/004.SeedTestData.sql` with: (a) a second `tenant.Incubators` row named `"Incubadora Norte"` (ExternalId `22222222-2222-2222-2222-222222222222`), (b) an `access.Users` row for `coord2@test.mentoory.com` hashed with the same password helper as the existing seed (`Test123!@#`), (c) an `access.RoleAssignments` row granting ProjectCoordinator on `Incubadora Norte`, (d) a `tenant.Projects` row `"Proyecto Norte Uno"` bound to the seeded KS template so its KS materializes at seed time, and (e) an optional project-scoped role assignment for `coord2` to `Proyecto Norte Uno`. All inserts idempotent. Verify via `dotnet build Mentoory.Db/MentooryDb.sqlproj`.
- [ ] T006 [P] Create `tests/Mentoory.Tests.E2E/Infrastructure/KnowledgeTestHelpers.cs` implementing the full API in `contracts/e2e-helpers.md` (public static class, `ContextSelection` record with static presets, `LoginAsync`/`SelectContextAsync`/`LoginAndSelectAsync`/`ScreenshotOnFailureAsync`/`WaitForSpanishMessageAsync`). Use the existing `LoginAndSelectContextAsync` in `KnowledgeTemplatesTests.cs` as the reference implementation. Ensure the dropdown-XHR wait pattern (`WaitForFunctionAsync("sel => sel.options.length > 1", ...)`) is internal and not callable-around.
- [ ] T007 Create `tests/Mentoory.Tests.E2E/Infrastructure/KnowledgeIntegrationHelpers.cs` exposing the API in `contracts/test-files.md` § Shared Integration Helpers: `CreateFormTemplateAsync`, `AddTopicToTemplateAsync`, `CountProjectFormsAsync`, `CountProjectKnowledgeStructuresAsync`, `CloneFormIntoProjectAsync`, plus `GetSeededKsTemplateExternalIdAsync` and `GetSeededBoundFormTemplateExternalIdAsync`. Every method takes a `WebApplicationFactory<Program>` (which `PlaywrightFixture` is) and uses `Services.CreateScope()` + `IMediator.Send` / `DbContext` queries. No raw SQL.
...
- [ ] T009 [US1] Extend existing `Templates_PageLoads_ShowsSeededTemplate` in `tests/Mentoory.Tests.E2E/Tests/KnowledgeTemplatesTests.cs` with the archived/active-state assertion and "Nueva plantilla" action-reachable assertion to cover the full US1-1 scenario (replaces current partial coverage). Method stays named the same.
- [ ] T010 [US1] Add `Templates_ListSurface_ShowsArchivedStateAndNewButton` to `tests/Mentoory.Tests.E2E/Tests/KnowledgeTemplatesTests.cs` covering the remainder of US1-1 (visible archived indicator + "Mostrar archivadas" toggle presence).
- [ ] T011 [US1] Extend existing `TemplateDetail_PageLoads_ShowsTree` in `tests/Mentoory.Tests.E2E/Tests/KnowledgeTemplatesTests.cs` with assertions that all four hierarchy levels (Module, Topic, Subject, Resource) render in the tree with their Spanish names. Covers US1-2.
- [ ] T012 [US1] Add `TemplateDetail_AddNodes_PersistsAfterReload` to `tests/Mentoory.Tests.E2E/Tests/KnowledgeTemplatesTests.cs`: create a test-local template, add one Module then one Topic under it then one Subject under the topic then one Resource (ResourceType=Video) under the subject, reload, assert all present. Covers US1-3.
- [ ] T013 [US1] Add `TopicPriorityRanges_SaveAndReload_PersistsThreeBands` to `tests/Mentoory.Tests.E2E/Tests/KnowledgeTemplatesTests.cs`: on a test-local template's topic, fill High 80–100 / Medium 50–79 / Low 0–49, save, reload, assert all three render in order. Covers US1-4.
- [ ] T014 [US1] Add `TopicPriorityRanges_OverlappingBands_ShowsValidationError` to `tests/Mentoory.Tests.E2E/Tests/KnowledgeTemplatesTests.cs`: fill overlapping bands (High 70–100, Medium 65–80), submit, assert Spanish field-level error surfaces AND previous bands preserved. Covers US1-5.
- [ ] T015 [US1] Add `TemplateModules_Reorder_PersistsAfterReload` to `tests/Mentoory.Tests.E2E/Tests/KnowledgeTemplatesTests.cs`: on a test-local template with two modules, reorder via the existing UI controls, reload, assert new order. Covers US1-6.
- [ ] T016 [US1] Add `Template_ArchiveAndUnarchive_ReflectsInListToggles` to `tests/Mentoory.Tests.E2E/Tests/KnowledgeTemplatesTests.cs`: create a test-local template, archive it, toggle "Mostrar archivadas", assert visible; unarchive, assert back to active list. Covers US1-7.
- [ ] T017 [US1] Add `Template_HardDelete_BlockedWhenProjectCloneExists` to `tests/Mentoory.Tests.E2E/Tests/KnowledgeTemplatesTests.cs`: attempt hard-delete on the seeded `Emprendimiento Básico` template (which has clones via the seeded project), assert Spanish block message containing "archivar en lugar de eliminar" (or exact message from FR-K01), assert template still in list. Covers US1-8.
- [ ] T018 [US1] Replace existing `Templates_CoordinatorCannotAccess` assertion shape in `tests/Mentoory.Tests.E2E/Tests/KnowledgeTemplatesTests.cs` with a comment pointing to `KnowledgeAuthorizationTests.ProtectedRoutes_CoordinatorDenied_ForTemplateRoutes` (T044) that covers this plus the `/Create`, `/{id}/Edit`, `/{id}/Modules/**` routes. Keep the simpler existing test as a breadcrumb smoke.
...
- [ ] T019 [US2] Keep existing `Projects_PageLoads_ShowsMaterializedKs` in `tests/Mentoory.Tests.E2E/Tests/KnowledgeProjectStructureTests.cs` (already migrated to shared helper in T008). Covers US2-1. Add a comment linking to US2 scenario 1.
- [ ] T020 [US2] Keep existing `Projects_NoCloneFromTemplateButton_Phase9Regression` in `tests/Mentoory.Tests.E2E/Tests/KnowledgeProjectStructureTests.cs`. Covers US2-5. Verify after T008 migration.
- [ ] T021 [US2] Keep existing `CloneFromTemplate_LegacyRoute_NoLongerAccessible` in `tests/Mentoory.Tests.E2E/Tests/KnowledgeProjectStructureTests.cs`. Covers US2-6. Verify after T008 migration.
- [ ] T022 [US2] Extend existing `ProjectStructureDetail_PageLoads_ShowsTree` in `tests/Mentoory.Tests.E2E/Tests/KnowledgeProjectStructureTests.cs` to assert (a) all four hierarchy levels render, (b) the `SyncMode` badge shows `"Desconectado"` (Disconnected default), (c) specific seeded Spanish names appear in the tree. Covers US2-4. Also implicitly covers the US4-2 "rename baseline" via the known-name assertion.
- [ ] T023 [US2] Add `ProjectStructureDetail_TreeContainsClonedNames` to `tests/Mentoory.Tests.E2E/Tests/KnowledgeProjectStructureTests.cs` asserting the seeded `Finanzas` and `Mercadeo` topics (or whichever the seed contains) are present. Acts as a regression guard for US4-2's "rename doesn't leak" baseline.
- [ ] T024 [US2] Keep existing `ProjectCreationTests.CreateProject_PageLoads_WithNewFields`, `CreateProject_ValidSubmission_RedirectsToProjectsList`, `CreateProject_MissingKsTemplate_ShowsValidationError`, `CreateProject_IsPublicCheckbox_DefaultsToUnchecked` in `tests/Mentoory.Tests.E2E/Tests/ProjectCreationTests.cs`. These cover US2 scenarios 2+3 already. Verify after T008 migration.
...
- [ ] T025 [P] [US3] Create `tests/Mentoory.Tests.E2E/Tests/KnowledgeFormCloneCascadeTests.cs` with the xUnit + `[Collection(E2ETestCollection.Name)]` scaffold and a constructor that takes `PlaywrightFixture`.
- [ ] T026 [US3] Add `CloneCompatibleForm_HappyPath_CreatesProjectForm` to `tests/Mentoory.Tests.E2E/Tests/KnowledgeFormCloneCascadeTests.cs`: log in as `coord1`, navigate to Diagnostic forms list, pick the seeded `"Diagnóstico Básico de Emprendimiento"` (seeded in T004), clone into project, assert success toast + new `ProjectForm` row visible. Covers US3-1 at the UI layer.
- [ ] T027 [US3] Add `CloneMismatchedForm_ShowsPhase9MismatchError` to `tests/Mentoory.Tests.E2E/Tests/KnowledgeFormCloneCascadeTests.cs`: use `KnowledgeIntegrationHelpers.CreateFormTemplateAsync` to build a test-local FormTemplate bound to a *different* KS template (create a second KS template inline via the helper too), attempt clone into the seeded project, assert the substring `"Este formulario está diseñado para una estructura de conocimiento diferente"` is present in the page content, assert `KnowledgeIntegrationHelpers.CountProjectFormsAsync` for that project is unchanged from pre-clone. Covers US3-2.
- [ ] T028 [P] [US3] Add `CloneFormTemplate_NullBinding_NoKnowledgeCascade` to `tests/Mentoory.Tests.Integration/Knowledge/DiagnosticCascadeRoundTripTests.cs`: use the existing `SetupProjectWithKsTemplateAsync` helper to create project + KS template, then build a FormTemplate with `DefaultKnowledgeStructureTemplateExternalId = null`, clone into the project via `CloneFormTemplateCommand`, assert (a) clone succeeds, (b) `KnowledgeDbContext.Set<KS>().Count(s => s.ProjectId == projectId) == 1`, (c) no new `ProjectForm` beyond the expected one. Covers US3-3.
- [ ] T029 [P] [US3] Add `KnowledgeIntegrationHelpers_UtilityContract_RegressionGuard` to `tests/Mentoory.Tests.Integration/Knowledge/DiagnosticCascadeRoundTripTests.cs`: a small round-trip test that uses `KnowledgeIntegrationHelpers.AddTopicToTemplateAsync` and `CreateFormTemplateAsync` (or their command-layer equivalents) to prove the helpers commit to the same DB the `KnowledgeDbContext` reads. Runs in under 1 second. Acts as fail-fast guard on helper drift.
...
- [ ] T047 [US6] Add `TenantIsolation_CoordinatorB_CannotAccessCoordinatorAsKs` to `tests/Mentoory.Tests.E2E/Tests/KnowledgeAuthorizationTests.cs`: login as `coord2@test.mentoory.com` (Incubadora Norte), obtain `coord1`'s project KS ExternalId via integration helper, GET its detail route, assert ≥400 OR body contains no names from coord1's tree. Covers US6-3.
