...
- [x] **T001** Create directory `Mentoory.Web/wwwroot/img/banners/`.
- [x] **T002 [P]** Author `banners/dashboard.svg` — accent #E07850, **mosaic** motif.
- [x] **T003 [P]** Author `banners/proyectos.svg` — accent #F5B731, **chevron** motif.
- [x] **T004 [P]** Author `banners/conocimiento.svg` — accent #D946A8, **mosaic** motif.
- [x] **T005 [P]** Author `banners/diagnostico.svg` — accent #4299E1, **diagonal** motif.
- [x] **T006 [P]** Author `banners/personas.svg` — accent #E07850, **chevron** motif.
- [x] **T007 [P]** Author `banners/incubadoras.svg` — accent #D946A8, **diagonal** motif.
- [x] **T008 [P]** Author `banners/auditoria.svg` — accent #1B2434 (slate), **mosaic** motif.
- [x] **T009 [P]** Author `banners/default.svg` — accent #E07850, **diagonal** motif (fallback).
...
- [x] **T010** Rewrite the shared `.page-banner` rule:
...
- [x] **T011** Per-slug rules (one per slug, mirroring `.header-band--{slug}`): set
...
- [x] **T012** `.page-banner__title`: confirm dark-on-light over the transparent left region; keep
  single-line truncation (`overflow/white-space/text-overflow`). Verify WCAG-AA (FR-009/SC-005).
- [x] **T013** `.page-banner__icon`: restyle as a faint **light/white** watermark legible over the
  bold right field (tune opacity ~0.22–0.30); stays decorative, non-interactive.
- [x] **T014** Responsive `@media (max-width: 767.98px)`: set `--banner-art: none` and hide
  `.page-banner__icon` (keep the colour field + title) — FR-020.
- [x] **T015** `@media print`: keep `.page-banner { display: none; }` (FR-022).
...
- [x] **T016 [P]** Add `tests/Mentoory.Tests.Integration/Web/BannerAssetsServedTests.cs`: for each
...
- [x] **T017** Update `tests/Mentoory.Tests.E2E/Tests/PageBannerTests.cs`: change the height-band
...
- [x] **T018** Verify `PageBannerRenderTests` (markup contract) still passes **unchanged** — this is
...
- [x] **T019** `dotnet build` clean (zero warnings — `TreatWarningsAsErrors`).
- [x] **T020** Run `dotnet test tests/Mentoory.Tests.Integration` and
  `tests/Mentoory.Tests.E2E` (banner + header-band suites) green.
- [x] **T021 [P]** Run `/simplify` over the changed CSS block; confirm no dead rules, no leftover
  023 gradient artefacts, comments updated to reflect the redesign (reference 024).
- [x] **T022 [P]** Manual pass per `quickstart.md` acceptance list (bold/distinct, height, contrast,
