CREATE TABLE [access].[AuthSessions]
(
    ...
    CONSTRAINT [FK_AuthSessions_Users] FOREIGN KEY ([UserId]) REFERENCES [access].[Users] ([Id])
)
GO

CREATE UNIQUE NONCLUSTERED INDEX [IX_AuthSessions_SessionToken]
    ON [access].[AuthSessions] ([SessionToken])
    WHERE [IsActive] = 1
GO

CREATE NONCLUSTERED INDEX [IX_AuthSessions_UserId_IsActive]
    ON [access].[AuthSessions] ([UserId], [IsActive])
